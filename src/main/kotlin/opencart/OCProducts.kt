...
internal data class OCProducts(private val product: Product) :
    ...
    override val data: List<LinkedHashMap<String, Any>> get() {
        return listOf(linkedMapOf(
            ...
            "meta_description(ru-ru)" to "${product.name} ${product.model} ${product.description} ${product.subcategory} купить с доставкой в магазине Ральф-Интерьер",
            "meta_keywords(ru-ru)" to "${product.subcategory},${product.name},${product.model}",
            "stock_status_id" to 6,
            "store_ids" to "0",
            "layout" to "0:",
            "related_ids" to "",
            "tags(ru-ru)" to "${product.subcategory},${product.name}",
            "sort_order" to 1,
            "subtract" to true,
            "minimum" to 1
        ))
